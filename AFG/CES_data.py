...
def load_CES_data(base_folder='/root/jupyter', features_folders=['AVEC2019_CES_traindevel/audio_features_egemaps_xbow/'], targets=['arousal','valence','liking'], test_available=False):
    ...
    test_BR_x = np.empty((num_test_BR, max_seq_len, 0))
    test_CH_x = np.empty((num_test_CH, max_seq_len, 0))
    ...
    for features_folder in features_folders:
        ...
        if test_available:
            test_BR_x = np.concatenate( (test_BR_x, load_features(path_features=base_folder+features_folder, partition='Test_BR', num_inst=num_test_BR, max_seq_len=max_seq_len) ), axis=2)
            test_CH_x = np.concatenate( (test_CH_x, load_features(path_features=base_folder+features_folder, partition='Test_CH', num_inst=num_test_CH, max_seq_len=max_seq_len) ), axis=2)
    
    _                       , train_DE_y = load_labels(path_labels=base_folder+'AVEC2019_CES_traindevel/labels/', partition='Train_DE', num_inst=num_train_DE, max_seq_len=max_seq_len, targets=targets)
    _                       , train_HU_y = load_labels(path_labels=base_folder+'AVEC2019_CES_traindevel/labels/', partition='Train_HU', num_inst=num_train_HU, max_seq_len=max_seq_len, targets=targets)
    devel_DE_labels_original, _          = load_labels(path_labels=base_folder+'AVEC2019_CES_traindevel/labels/', partition='Devel_DE', num_inst=num_devel_DE, max_seq_len=max_seq_len, targets=targets)
    devel_HU_labels_original, _          = load_labels(path_labels=base_folder+'AVEC2019_CES_traindevel/labels/', partition='Devel_HU', num_inst=num_devel_HU, max_seq_len=max_seq_len, targets=targets)
    
    return train_DE_x, train_HU_x, train_DE_y, train_HU_y, devel_DE_x, devel_HU_x, devel_DE_labels_original, devel_HU_labels_original, test_BR_x, test_CH_x
